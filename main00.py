...
def get_ordered_original_data(orders, sort_by):
    #  0-web, 1-flange
    if sort_by == 0:
        sort = '%WEB%'
    else:
    ...
    sql = "select t.* from(SELECT  tblManifestTP.ProjID, tblPrtAssm.PrtNo AS PartNO," \
          ...
          " WHERE        (tblSubPrt.Descp LIKE '" + sort + "') AND " + select_orders(orders) + \
...
if __name__ == '__main__':
    ...
    sort_by = 0  #  0-web, 1-flange

    db = DBUnit('zyq2', 'zyq2', 'stlsojsvr04', 'MFGMISCSSQL')

    rs = get_ordered_original_data(orders, sort_by)
    get_sequence(rs)
